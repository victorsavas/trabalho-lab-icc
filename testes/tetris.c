...
#include "gameloop.h"                      // lógica do loop de eventos
...
int main() {
    ...
    while (falling) {         // Loop principal do jogo
        ...
        if (redraw && al_is_event_queue_empty(queue)) { // Redesenho da tela
            ...
            int sprite_x = 0;        // Coordenada X da imagem a ser exibida
            int sprite_y = 0;        // Coordenada Y da imagem a ser exibida

            for(int i=0; i<20; i++){
                for(int j=0; j<10; j++){
                    sprite_y = board[i * BOARD_COLS + j] - 1; // define a cor do bloco
                    al_draw_bitmap_region(sprite_sheet, sprite_x, 16 * sprite_y, SPRITE_SIZE, SPRITE_SIZE,
                                         (WIDTH - SPRITE_SIZE)/2 + (SPRITE_SIZE * j), HEIGHT/2 + SPRITE_SIZE + (SPRITE_SIZE * i), 0); // desenha o bloco
                    }
                }
